...
export default function SubjectSelectionInfo() {
  const [gcseSelections, setGcseSelections] = useState<Record<string, number | undefined>>({});
  const [aLevelSelections, setALevelSelections] = useState<Record<string, string | undefined>>({});

  const handleGCSEChange = (subject: string, isChecked: boolean, grade: number | undefined) => {
  ...
  const handleALevelChange = (subject: string, isChecked: boolean, grade: string | undefined) => {
            ...
            <div key={index} className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    handleGCSEChange(subject.name, e.target.checked, e.target.checked ? gcseGrades[0] : undefined)
                  }
                  className="sm:w-4 sm:h-4 w-5 h-5"
                />
                <label htmlFor={`gcse-${index}`} className="text-sm sm:text-base text-[#4A4C56]">
                  {subject.name}
                </label>
                {subject.mandatory && (
              ...
              {gcseSelections[subject.name] !== undefined && (
                <select
                  ...
                  onChange={(e) => handleGCSEChange(subject.name, true, Number(e.target.value))}
            ...
            <div key={index} className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    handleALevelChange(subject, e.target.checked, e.target.checked ? aLevelGrades[0] : undefined)
                  }
                  className="sm:w-4 sm:h-4 w-5 h-5"
                />
                <label htmlFor={`alevel-${index}`} className="text-sm sm:text-base text-[#4A4C56]">
                  {subject}
                </label>
              </div>
              {aLevelSelections[subject] !== undefined && (
                <select
                  ...
                  onChange={(e) => handleALevelChange(subject, true, e.target.value)}
                  className="border rounded px-2 py-1 text-sm sm:text-base"
                >
